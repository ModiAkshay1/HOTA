...
## 2. Per-frame raw and normalized similarity matrices $S^t$
...
flowchart LR
    ...
    A2 --> A3[Sum across frames to get C\_gp]
    A3 --> A4[Use N^G\_g and N^P\_p to get A\_gp]
    A4 --> A5[Multiply with S^t to get W^t]
    A5 --> A6[Hungarian matching Pi\_t]
    A6 --> A7[Keep only matches with S^t\_ij >= 0.5]
    A7 --> A8[Compute TP/FN/FP]
    A7 --> A9[Compute M^0.5\_gp]
    A7 --> A10[Compute L\_0.5]
    A8 --> A11[DetA\_0.5 = 0.7143]
    A9 --> A12[AssA\_0.5 = 0.53]
    A10 --> A13[LocA\_0.5 = 0.737]
    A11 --> A14[HOTA\_0.5 = 0.6153]
    A12 --> A14
```
...
3. **`matches\_count` is distributed**  
